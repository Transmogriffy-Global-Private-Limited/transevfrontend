...
const fetch2FAStatus = async () => {
    try {
      ...
      // Make the API request to check the 2FA status
      ...
      console.log("Full 2FA Status Response:", res);
  
      // Directly access the message if the response doesn't include 'status'
      const message = res.data.message;
  
      // Check if the message contains "enabled" to set the 2FA status
      ...
      console.error("Error fetching 2FA status:", err.response || err); // Log the full error
      if (err.response) {
        // If there's a response error (e.g., 401 or 500)
        alert(`Error: ${err.response.status} - ${err.response.data?.message || err.response.statusText}`);
      } else {
        // If the error doesn't have a response (e.g., network failure)
        alert("Failed to load 2FA status. Please check your internet connection.");
      }
    }
  };
