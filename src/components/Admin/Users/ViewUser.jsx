...
//                       <th
//                         className="py-3 px-6 text-left text-sm font-medium text-gray-300 cursor-pointer"
...
const UserList = () => {
  ...
  const [totalPages, setTotalPages] = useState(1);
  const [sortOrder, setSortOrder] = useState('asc');
  const [sortType, setSortType] = useState('date'); // date | name
  const [sidebarOpen, setSidebarOpen] = useState(true);
  ...
  // ✅ SORTING LOGIC (Recent users first)
  const sortedUsers = [...users].sort((a, b) => {
    if (sortType === 'date') {
                  ...
                  <tbody>
                    {sortedUsers.map((user) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">{user.user_number}</td>
