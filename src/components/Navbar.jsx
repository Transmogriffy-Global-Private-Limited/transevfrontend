    ...
    <>
      ...
      <nav
        className={`absolute top-0 left-0 w-full transition-all duration-300 ${
          // Background and text color logic based on page and popup
          isHomePage && !popupType
            ? 'bg-transparent text-white' // Home page: transparent bg, white text
            : 'bg-white text-black' // Other pages or popup: white bg, black text
  ...
  <Link to="/"> {/* Link to home page */}
    <img src={logo} alt="Company Logo" className="w-32 h-32" />
          ...
          <div className="flex space-x-12 text-xl font-bold ml-40">
            <div
              onMouseEnter={() => setPopupType('solutions')}
              className="relative"
            >
              <Link
                to="/solution"
            ...
            <div
              onMouseEnter={() => setPopupType('about')}
              className="relative"
            >
              <Link
                to="/about"
            ...
            <div
              onMouseEnter={() => setPopupType(null)} // Close popup when hovering over News
              className="relative"
            >
            ...
            {popupType === 'solutions' &&
              [slide1, slide2, slide3, slide4].map((image, index) => (
                <div key={index} className="relative flex flex-col items-center mx-4 mb-4 group">
                  ...
                  />
                  <span className="mt-2 font-semibold">{solutionCaptions[index]}</span> {/* Caption under the image */}
                  ...
                  <div
                    ...
                    onClick={() => handleImageClick(`/solution/${index + 1}`)} // Redirect to solution page on click
                  >
              ...
              [about1, about2].map((image, index) => (
                <div key={index} className="relative flex flex-col items-center mx-4 mb-4 group">
                  <img
                    ...
                    className="w-[400px] h-[300px] rounded-lg shadow-md transition-transform duration-300 transform hover:scale-105"
                  />
                  <span className="mt-2">{aboutCaptions[index]}</span> {/* Caption under the image */}
                  {/* Yellow Circle on Hover (centered in the image) */}
                  <div
                    className="absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 ease-in-out"
                    onClick={() => handleImageClick(`/about/${index + 1}`)} // Redirect to about page on click
                  >
                    <div className="bg-yellow-300 w-12 h-12 rounded-full flex justify-center items-center ml-60 mt-40">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
